...
<div style="margin-left: 2px;">
    <h2 style="margin-top: 12px;" class="alert alert-success"> Bible Study Groups</h2><br>
    <div class="row">
        <div class="col-12">
          <a href="javascript:void(0)" class="btn btn-success mb-2" id="create-new-post">Add BS Group</a> 
        ...
        <form id="postForm" name="postForm" class="form-horizontal">
           <input type="hidden" name="post_id" id="post_id">
            <div class="form-group">
                ...
                <div class="col-sm-12">
                    <input type="text" class="form-control" id="bs" name="bs" value="" required="">
                </div>
            </div>
...
<script>
  $(document).ready(function () {
    ...
    $('#create-new-post').click(function () {
        ...
        $('#postCrudModal').html("Add New BS Group");
        $('#ajax-crud-modal').modal('show');
    });
 
    $('body').on('click', '#edit-post', function () {
      var post_id = $(this).data('id');
      $.get('ajax-bs/'+post_id+'/edit', function (data) {
         $('#postCrudModal').html("Edit BS Group");
 ...
 if ($("#postForm").length > 0) {
     ...
     submitHandler: function(form) { 
      ...
      $.ajax({
          ...
          url: "{{ route('ajax-bs.store') }}",
          type: "POST",
          dataType: 'json',
          success: function (data) {
              var post = '<tr id="post_id_' + data.id + '"><td>' + data.id + '</td><td>' + data.bs +  '</td>';
