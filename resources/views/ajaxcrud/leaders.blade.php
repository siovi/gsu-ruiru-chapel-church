...
<div style="margin-left: 2px;">
    <h2 style="margin-top: 12px;" class="alert alert-success">Church Leaders</h2><br>
    <div class="row">
        <div class="col-12">
          <a href="javascript:void(0)" class="btn btn-success mb-2" id="create-new-post">Add New Leaders</a> 
...
<script>
  $(document).ready(function () {
    ...
    $('#create-new-post').click(function () {
        ...
        $('#postCrudModal').html("Add New Leader");
        $('#ajax-crud-modal').modal('show');
    });
 
    $('body').on('click', '#edit-post', function () {
      var post_id = $(this).data('id');
      $.get('ajax-leaders/'+post_id+'/edit', function (data) {
         $('#postCrudModal').html("Edit Leader");
 ...
 if ($("#postForm").length > 0) {
     ...
     submitHandler: function(form) {
      ...
      $.ajax({
          ...
          url: "{{ route('ajax-leaders.store') }}",
          type: "POST",
          dataType: 'json',
          success: function (data) {
              var post = '<tr id="post_id_' + data.id + '"><td>' + data.id + '</td><td>' + data.name + '</td><td>' + data.gender +'</td><td>'+ data.dob + 
              '</td><td>' + data.residence + '</td><td>' + data.position + '</td><td>' + data.phoneno + '</td><td>' + data.email +
                '</td>';
