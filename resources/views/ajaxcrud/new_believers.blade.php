        ...
        <form id="postForm" name="postForm" class="form-horizontal">
           <input type="hidden" name="post_id" id="post_id">
            ...
            <div class="form-group">
                ...
                <div class="col-sm-12">
                <input class="form-control" id="date" name="date"  type="date" placeholder = "Date Of Service..." required>
                </div>
            </div>
            <div class="form-group">
...
<script>
  $(document).ready(function () {
    ...
    $('#create-new-post').click(function () {
        ...
        $('#postCrudModal').html("Add New Believer");
        $('#ajax-crud-modal').modal('show');
    });
 
    $('body').on('click', '#edit-post', function () {
      var post_id = $(this).data('id');
      $.get('ajax-new_believers/'+post_id+'/edit', function (data) {
         $('#postCrudModal').html("Edit Believer");
    ...
    $('body').on('click', '.delete-post', function () {
        ...
        $.ajax({
            type: "DELETE",
            url: "{{ url('ajax-new_believers')}}"+'/'+post_id,
 ...
 if ($("#postForm").length > 0) {
     ...
     submitHandler: function(form) {
      ...
      $.ajax({
          ...
          url: "{{ route('ajax-new_believers.store') }}",
          type: "POST",
          dataType: 'json',
          success: function (data) {
              var post = '<tr id="post_id_' + data.id + '"><td>' + data.id + '</td><td>' + data.name + '</td><td>' + data.gender +'</td><td>' + data.dob + '</td><td>' + data.date +'</td><td>' + data.phoneno + '</td><td>' + data.email + '</td><td>' + data.status + '</td>';
