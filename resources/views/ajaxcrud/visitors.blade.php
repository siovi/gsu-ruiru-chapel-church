...
<div style="margin-left: 2px;">
    <h2 style="margin-top: 12px;" class="alert alert-success">Visitors</h2><br>
    <div class="row">
        <div class="col-12">
          <a href="javascript:void(0)" class="btn btn-success mb-2" id="create-new-post">Add New Visitors</a> 
...
<div class="modal-content">
    ...
    <div class="modal-body">
        <form id="postForm" name="postForm" class="form-horizontal">
           <input type="hidden" name="post_id" id="post_id">
            ...
            <div class="form-group">
                <label class="col-sm-2 control-label">place_of_birth</label>
                <div class="col-sm-12">
                    <input class="form-control" id="place_of_birth" name="place_of_birth" value="" >
                </div>
            </div>
            <div class="form-group">
                <label for="name" class="col-sm-2 control-label">residence</label>
                <div class="col-sm-12">
                    <input type="text" class="form-control" id="residence" name="residence" value="" >
                </div>
            </div>
 
            <div class="form-group">
            <label for="name" class="col-sm-2 control-label">Select Event Attended</label>
                                  <div class="col-sm-12">
  ...
  <option value="Week Day Service">Week Day Service</option>
  ...
  <option value="Others">Others</option>
            ...
            <div class="form-group">
                <label class="col-sm-2 control-label"> phoneno</label>
                <div class="col-sm-12">
                    <input class="form-control" id="phoneno" name="phoneno" value="" >
                </div>           
            </div>   
            <div class="form-group">
...
<script>
  $(document).ready(function () {
    ...
    $('#create-new-post').click(function () {
        ...
        $('#postCrudModal').html("Add New Visitor");
        $('#ajax-crud-modal').modal('show');
    });
 
    $('body').on('click', '#edit-post', function () {
      var post_id = $(this).data('id');
      $.get('ajax-visitors/'+post_id+'/edit', function (data) {
         $('#postCrudModal').html("Edit Visitor");
 ...
 if ($("#postForm").length > 0) {
     ...
     submitHandler: function(form) {
      ...
      $.ajax({
          ...
          url: "{{ route('ajax-visitors.store') }}",
          type: "POST",
          dataType: 'json',
          success: function (data) {
              var post = '<tr id="post_id_' + data.id + '"><td>' + data.id + '</td><td>' + data.name + '</td><td>' + data.gender +'</td><td>'+ data.dob + 
              '</td><td>' + data.place_of_birth + '</td><td>' + data.residence + '</td><td>' + data.ministry + '</td><td>' + data.phoneno + '</td><td>' + data.email + 
                '</td>';
